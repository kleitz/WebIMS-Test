...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_type)){
        $status_err = "Please enter the Type.";     
    ...
    if(empty($name_err) && empty($name_err) && empty($status_err)){
        ...
        $sql = "INSERT INTO shelters (name, tactical,type) VALUES (?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sss", $param_name, $param_tactical, $param_type);
